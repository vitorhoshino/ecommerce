	 ...
	 if(count($results)=== 0 )
	{
		...
		else
		{
			...
			else
			{
				...
				$code = $iv = openssl_random_pseudo_bytes(openssl_cipher_iv_length('aes-256-cbc')); // generate 16  random bytes
...
//cadastro
		...
		$results = $sql->select("
			SELECT*
			FROM tb_orders a
			INNER JOIN tb_orderstatuts b USING(idstatus)
			INNER JOIN tb_carts c USING(idcart)
			INNER JOIN tb_users d ON d.iduser = a.iduser
			INNER JOIN tb_addresses e USING(idaddresses)
